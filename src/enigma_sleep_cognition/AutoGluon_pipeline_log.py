...
class AdMLPipeline:
    def __init__(self, X, y, df_train=None, num_cores=None, num_gpus=0, save_path=None, model=None, log_target=False):
        self.X = X
        self.y = y
    ...
    def autogluon_pipe(self):
        ...
        # ============================================================
        # 【新增】 Step A: Log 变换 (必须在 rgo_age 之前)
        # ============================================================
        if self.log_target:
            print(f"Applying Log transformation (np.log1p) to target: {self.y}")
            # 使用 log1p 防止 0 值报错
            self.df_train[self.y] = np.log1p(self.df_train[self.y])
    ...
    def test_perform_plot(self, df_test, fig_title=None):
        ...
        # 获取原始真实值 (Raw Truth)，用于最终计算 R2/MAE
        y_true_raw = df_test[self.y].values.copy()

        # 1. 模型预测 (此时得到的可能是 Log后的值，或者是 Log后的残差)
        df_test[self.X] = X_test
        y_pred = self.model.predict(df_test[self.X + [self.y]])  # Pandas Series
        y_pred = y_pred.values  # 转 numpy

        # ============================================================
        # 【修改】 还原预测值到原始空间 (Inverse Transform Pipeline)
        # ============================================================

        # 2. 如果做了 rgo_age，先把年龄效应加回来
        if self.rgo_age:
            age_test = df_test['Age_at_Scan'].values.reshape(-1, 1)
            age_test_poly = self.whole_age_poly.transform(age_test)
            age_effect = self.whole_age_lr.predict(age_test_poly)

            # 还原: 残差 + 年龄效应
            y_pred = y_pred + age_effect
        ...
        # 此时 y_pred 已经是原始单位（秒/分数）了，y_true_raw 也是原始单位
        # 我们手动计算指标，而不再使用 model.evaluate (因为 model.evaluate 可能会基于 Log 空间计算)

        # ============================================================
        # 计算指标 (使用 sklearn 手动计算)
        # ============================================================
        r2 = r2_score(y_true_raw, y_pred)
        rmse = root_mean_squared_error(y_true_raw, y_pred)
        mae = mean_absolute_error(y_true_raw, y_pred)
        corr = np.corrcoef(y_true_raw, y_pred)[0, 1]
        corr_spearman, _ = spearmanr(y_true_raw, y_pred)

        # score_val 这里比较尴尬，因为 evaluate 算的是 Log 空间的
        # 我们直接用 R2 作为 score_val 返回
        score_val = r2

        # Create a DataFrame for Seaborn
        data = pd.DataFrame({
            'True Values': y_true_raw,  # 使用原始真实值
            'Predicted Values': y_pred  # 使用还原后的预测值
        })
