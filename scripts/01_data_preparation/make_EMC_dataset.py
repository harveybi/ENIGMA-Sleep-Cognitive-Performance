...
sys.path.append('/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/EMC_validation/lib/')  # edit to your path
import utils
...
templates_path = '/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/EMC_validation/data_extract_scripts/'  # edit to your path
...
# Schaefer
example_schaefer_ct_df = pd.read_csv(templates_path + 'control_thickness_Schaefer.csv')
...
renamed_schaefer_ct_df_columns = [col.replace('lh_7Networks_', '').replace('rh_7Networks_', '') for col in
                                  filtered_columns]
# DK
example_dk_ct_df = pd.read_csv(templates_path + 'control_thickness_DK.csv')
renamed_dk_ct_df_columns = [col for col in example_dk_ct_df.columns if
                            col not in ['subject_ID', 'BrainSegVolNotVent', 'eTIV']]

# rename surface area
# Schaefer
example_schaefer_sa_df = pd.read_csv(templates_path + 'control_area_Schaefer.csv')
...
renamed_schaefer_sa_df_columns = [col.replace('lh_7Networks_', '').replace('rh_7Networks_', '') for col in
                                  filtered_columns]
# DK
example_dk_sa_df = pd.read_csv(templates_path + 'control_area_DK.csv')
