...
def preprocess_emc_sleep_cov(df, *, utils_module, feature_cols):
    """
    Minimal preprocessing for Sleep_Cov only.

    No brain normalization.
    No shuffled features.
    No APOE-specific feature-combination preprocessing.
    """
    df = df.copy()
...
def make_emc_site_plots(
    *,
    ...
    exp_feature_names = (
        ...
        if getattr(explanation, "feature_names", None) is not None
        else list(feature_names)
    )
    exp_pretty = pretty_names(exp_feature_names, label_replacements)
...
def make_emc_cluster_plots(
    *,
    ...
    for cl in unique_labels:
        ...
        exp_feature_names = (
            list(exp_cl.feature_names)
            if getattr(exp_cl, "feature_names", None) is not None
            else list(feature_names)
        )
        exp_pretty = pretty_names(exp_feature_names, label_replacements)
...
def save_plot_ready_artifacts(
    *,
    ...
    if shap_values.shape[0] != len(labels):
        raise ValueError(
            f"SHAP row mismatch: shap_values={shap_values.shape[0]}, labels={len(labels)}"
        )

    if shap_values.shape[1] != len(feature_names):
        raise ValueError(
            f"SHAP feature mismatch: shap_values has {shap_values.shape[1]} columns, "
            f"feature_names has {len(feature_names)} columns."
        )
    ...
    if len(ivs) != len(labels):
        raise ValueError(
            f"SHAP-IQ row mismatch: ivs={len(ivs)}, labels={len(labels)}"
        )
