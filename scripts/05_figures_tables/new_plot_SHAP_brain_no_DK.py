...
MODEL  = "AutoGluon_no_DK"      # 'AutoGluon' or 'XGBoost'
TARGET = ("Memory")         # 'Stroop' or 'Memory'
...
def make_horizontal_colorbar(
    ...
    ticks_position: str = "bottom",  # 'bottom' or 'top'
    ax_rect: tuple[float, float, float, float] = (0.08, 0.40, 0.84, 0.25),  # [l,b,w,h] in figure coords
):
    """
    Standalone horizontal colorbar (no data axes).
    """
    import matplotlib as mpl
    import matplotlib.pyplot as plt
    ...
    # ticks & formatting (endpoints included)
    if n_ticks is not None:
        n_ticks = max(2, int(n_ticks))
        cb.locator = LinearLocator(n_ticks)
        cb.update_ticks()
    if tick_fmt is not None:
        cb.formatter = FuncFormatter(lambda x, pos: tick_fmt.format(x=x))
        cb.update_ticks()
    cb.ax.tick_params(pad=tickpad)
...
def top_k_features_by_mas(exp: shap.Explanation, k: int = 40) -> list[tuple[str, float]]:
    """Return [(feature_name, mean|SHAP|), ...] sorted desc and truncated to k."""
    s = mean_abs_shap_by_feature(exp).sort_values(ascending=False)
    s = s.iloc[:k]
    return list(zip(s.index.tolist(), s.values.tolist()))

def save_top_k_feature_names(exp: shap.Explanation,
                             outfile: Path,
                             k: int = 40,
                             pretty: bool = False,
                             sep: str = ", ") -> list[str]:
    """
    Save the top-k feature NAMES separated by `sep` on a single line.
    - pretty=False saves raw feature names (reproducible);
      pretty=True applies LABEL_REPLACEMENTS.
    """
    pairs = top_k_features_by_mas(exp, k)
    names = [LABEL_REPLACEMENTS.get(n, n) if pretty else n for n, _ in pairs]
    outfile.parent.mkdir(parents=True, exist_ok=True)
    with open(outfile, "w", encoding="utf-8") as f:
        f.write(sep.join(names) + "\n")
    return names

def save_top_k_table(exp: shap.Explanation,
                     outfile_csv: Path,
                     k: int = 40,
                     pretty: bool = True,
                     percent_of: str = "topk",     # "topk" or "all"
                     val_decimals: int = 6,
                     pct_decimals: int = 2,
                     normalize_100: bool = True) -> pd.DataFrame:
    """
    Save a CSV with columns: feature, mean_abs_shap, percent (0–100).
    - percent_of="topk": denominator=sum of top-k mean|SHAP| (matches your figure).
      percent_of="all":  denominator=sum over ALL features' mean|SHAP|.
    - pretty=True applies LABEL_REPLACEMENTS to names.
    - normalize_100 ensures the first row absorbs rounding so total≈100% for "topk".
    """
    pairs = top_k_features_by_mas(exp, k)  # [(name, val)] sorted desc
    names_raw = [n for n, _ in pairs]
    vals = np.array([float(v) for _, v in pairs], dtype=float)

    # denominator for %s
    if percent_of == "all":
        denom = float(mean_abs_shap_by_feature(exp).sum())
    else:  # "topk"
        denom = float(vals.sum())

    if denom <= 0 or not np.isfinite(denom):
        pcts = np.zeros_like(vals)
    else:
        pcts = 100.0 * (vals / denom)

    # absorb rounding into the first row so total=100% (only meaningful for "topk")
    if normalize_100 and percent_of == "topk" and pcts.size > 0:
        pcts[0] = 100.0 - float(pcts[1:].sum())

    feat_col = [LABEL_REPLACEMENTS.get(n, n) if pretty else n for n in names_raw]

    df = pd.DataFrame({
        "feature": feat_col,
        "mean_abs_shap": np.round(vals, val_decimals),
        "percent": np.round(pcts, pct_decimals),
    })

    outfile_csv.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(outfile_csv, index=False)
    print(f"[ok] saved top-{k} table → {outfile_csv}")
    return df
...
for cohort in COHORTS:
    ...
    if FEATURE_COMB == "Sleep_Cov_Brain":
        ...
        make_horizontal_colorbar(
            ...
            n_ticks=6,
            tick_fmt="{x:.2f}",
            outfile=str(cbar_dir / f"{MODEL}_{TARGET}_{cohort}_colorbar_horizontal.png")
        )
        ...
        TOP_K = 40
        top_dir = OUT_DIR / "top_features"
        top_dir.mkdir(parents=True, exist_ok=True)

        save_top_k_feature_names(
            exp,
            outfile=top_dir / f"{MODEL}_{TARGET}_{cohort}_top{TOP_K}_names.txt",
            k=TOP_K,
            pretty=True,  # set True if you want display names
            sep=", "
        )

        save_top_k_table(
            exp,
            outfile_csv=top_dir / f"{MODEL}_{TARGET}_{cohort}_top{TOP_K}.csv",
            k=TOP_K,
            pretty=True,  # set False if you need raw column names
            percent_of="topk",  # or "all" if you prefer % of total features
            val_decimals=6,
            pct_decimals=2,
            normalize_100=True
        )
