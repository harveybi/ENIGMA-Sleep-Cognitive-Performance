...
class AdMLPipeline:
    ...
    def xgboost_objective(self, trial, X_train, y_train):
        ...
        #     'n_jobs': 1
        # }
        # --- Define Feature Thresholds (Adjust as needed) ---
        LOW_FEAT_THRESHOLD = 30
        HIGH_FEAT_THRESHOLD = 200

        # --- Conditional Hyperparameter Ranges ---

        # 1. max_depth: Deeper trees less useful/more risky for low features
        if num_features <= LOW_FEAT_THRESHOLD:
            max_depth = trial.suggest_int('max_depth', 3, 10)
        elif num_features <= HIGH_FEAT_THRESHOLD:
            max_depth = trial.suggest_int('max_depth', 5, 15)
        else:  # High features
            max_depth = trial.suggest_int('max_depth', 7, 20)  # Avoid extremes > 20

        # 2. colsample_bytree: Feature sampling ratio per tree
        if num_features <= 10:  # Very low features
            # Ensure a minimum number/fraction of features are sampled
            # e.g., sample at least 4 features or 60%
            min_colsample = max(0.6, min(1.0, 4 / num_features))
            colsample_bytree = trial.suggest_float('colsample_bytree', min_colsample, 1.0)
        elif num_features <= HIGH_FEAT_THRESHOLD:  # Low/Medium features
            colsample_bytree = trial.suggest_float('colsample_bytree', 0.5, 1.0)  # Standard range
        else:  # High features
            # Allow slightly lower sampling potentially beneficial for high dim
            colsample_bytree = trial.suggest_float('colsample_bytree', 0.4, 1.0)

        # 3. n_estimators: Maybe allow more trees for high features?
        if num_features > HIGH_FEAT_THRESHOLD:
            n_estimators = trial.suggest_int('n_estimators', 100, 1500)  # Higher min/max maybe needed
        else:
            n_estimators = trial.suggest_int('n_estimators', 50, 1000)  # Standard range

        # --- Base Parameter Space (Less Dependent on Feature Count) ---
        param = {
            'tree_method': 'hist',
            # Regularization often more important for high features, but wide range handles this
            'lambda': trial.suggest_float('lambda', 1e-3, 100.0, log=True),  # Use suggest_float with log=True
            'alpha': trial.suggest_float('alpha', 1e-3, 100.0, log=True),  # Use suggest_float with log=True
            'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.3),
            'min_child_weight': trial.suggest_int('min_child_weight', 1, 20),
            'subsample': trial.suggest_float('subsample', 0.5, 1.0),
            'n_jobs': 1,  # n_jobs for the individual XGBoost training process
        ...
        _model = xgb.XGBRegressor(**param, random_state=42, enable_categorical=True)
        ...
        _scores = cross_val_score(_model, X_train, y_train, scoring=make_scorer(r2_score), cv=cv)  # n_jobs=inner_cv_n_jobs
        print(f"[Optuna Trial {trial.number}] cross_val_score finished in {time.time() - start_cv_time:.2f} sec. Result: {np.mean(_scores):.4f}")
    ...
    def xgboost_cross_val_iteration(self, repeat_fold, train_index, test_index):
        ...
        try:
            ...
            study.optimize(lambda trial: self.xgboost_objective(trial, X_train, y_train), n_trials=150, n_jobs=1)  # n_trials=150
    ...
    def xgboost_pipe(self, stratified_label=None, joblib_htcondor_path=None, joblib_log_path=None):
        ...
        throttle_config = [25, 50] if enable_nested_parallelism else [num_jobs]
        ...
        with parallel_config(
                ...
                request_disk="1GB",
                request_memory="2Gb",
                throttle=throttle_config,  # Throttle for the outer jobs only
                export_metadata=True,
                shared_data_dir=joblib_htcondor_path,
                log_dir_prefix=joblib_log_path,
                max_recursion_level=max_recursion,  # IMPORTANT: Disable nesting
                n_jobs=-1,
        ):
        ...
        print("\nCV results summary:")
        print(scores_df[['repeat', 'fold', 'test_r2', 'test_r_corr']].describe())
        print(f"\nMean R2: {scores_df['test_r2'].mean():.4f}, Mean r_corr: {scores_df['test_r_corr'].mean():.4f}")
        ...
        # save the whole_X_train_preprocessor, whole_age_poly, and whole_age_lr by pickle
        with open(self.save_path + "whole_X_train_preprocessor.pkl", "wb") as f:
            pickle.dump(self.whole_X_train_preprocessor, f)
        with open(self.save_path + "whole_age_poly.pkl", "wb") as f:
            pickle.dump(self.whole_age_poly, f)
        with open(self.save_path + "whole_age_lr.pkl", "wb") as f:
            pickle.dump(self.whole_age_lr, f)
...
joblib_htcondor_path = "/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/XGBoost/joblib_htcondor/" + f"{target}/" + f"{feature_comb}"
if not os.path.exists(joblib_htcondor_path):
    os.makedirs(joblib_htcondor_path)
joblib_log_path =  "/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/XGBoost/logs/" + f"{target}/" + f"{feature_comb}"
...
scores, model = ml_pipe_instance.xgboost_pipe(stratified_label='Group_Age_SEX', joblib_htcondor_path=joblib_htcondor_path, joblib_log_path=joblib_log_path)
