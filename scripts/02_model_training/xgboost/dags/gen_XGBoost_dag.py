...
dag_file = "dag_XGBoost_shap.dag"

with open(dag_file, 'w') as f:
    for target in targets:
        for feature_comb in feature_combs:
            # Define XGBoost job
            f.write(f"JOB XGBoost_{target}_{feature_comb} submit_dag_XGBoost_SHIP_Stroop_NAI.submit\n")
            f.write(
                f"VARS XGBoost_{target}_{feature_comb} feature_comb=\"{feature_comb}\" target=\"{target}\" req_cpu=\"4\"\n")

    for target in targets:
        for feature_comb in feature_combs:
            # Define SHAP job
            f.write(f"JOB SHAP_{target}_{feature_comb} submit_dag_SHAP_XGBoost_SHIP_Stroop_NAI.submit\n")
            f.write(f"VARS SHAP_{target}_{feature_comb} feature_comb=\"{feature_comb}\" target=\"{target}\"\n")

    for target in targets:
        for feature_comb in feature_combs:
            # Define the parent-child relationship
            f.write(f"PARENT XGBoost_{target}_{feature_comb} CHILD SHAP_{target}_{feature_comb}\n")
