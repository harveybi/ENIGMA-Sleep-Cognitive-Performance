...
dag_file = "dag_SVM-rbf_shap.dag"

with open(dag_file, 'w') as f:
    for target in targets:
        for feature_comb in feature_combs:
            # Define SVM-rbf job
            f.write(f"JOB SVM-rbf_{target}_{feature_comb} submit_dag_SVM-rbf_SHIP_Stroop_NAI.submit\n")
            f.write(
                f"VARS SVM-rbf_{target}_{feature_comb} feature_comb=\"{feature_comb}\" target=\"{target}\" req_cpu=\"8\"\n")

    for target in targets:
        for feature_comb in feature_combs:
            # Define SHAP job
            f.write(f"JOB SHAP_{target}_{feature_comb} submit_dag_SHAP_SVM-rbf_SHIP_Stroop_NAI.submit\n")
            f.write(f"VARS SHAP_{target}_{feature_comb} feature_comb=\"{feature_comb}\" target=\"{target}\"\n")

    for target in targets:
        for feature_comb in feature_combs:
            # Define the parent-child relationship
            f.write(f"PARENT SVM-rbf_{target}_{feature_comb} CHILD SHAP_{target}_{feature_comb}\n")
