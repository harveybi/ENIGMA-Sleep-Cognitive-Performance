...
import utils
import htcondor_ml_pipeline
from htcondor_ml_pipeline import MLPipeline
...
# %%
# Argument parsing
parser = argparse.ArgumentParser(description='Random Forest, SHIP_Trend dataset.')
...
print(f"\nStarting Random Forest pipeline for {target} prediction with feature combination {feature_comb}.\n")

# %%
data_save_path = '/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Data/'
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Results/rf/{target}/SHIP_Trend_HTCondor/'
...
with parallel_config(
    ...
    scores, model = ml_pipe_instance.rf_pipe(stratified_label='Group_Age_SEX')
...
#
# def generate_kfold(_df_train, _y=None, n_splits=5, random_state=0, stratified=False, n_repeats=1):
#     # kf = None
#     X_data = _df_train
#     y_data = _df_train[_y] if y is not None else None
#
#     if stratified and (_y is not None):
#         if n_repeats > 1:
...
#
# def rf_pipe(_df_train, _X, _y, stratified_label=None):
#     X_types = {"features": X}
...
#         cv_splitter = generate_kfold(_df_train, stratified_label, n_splits=5, random_state=42,
#                                           stratified=True, n_repeats=10)  # 5, 10
#     else:
#         cv_splitter = generate_kfold(_df_train, None, n_splits=5, random_state=42, stratified=False,
#                                           n_repeats=10)  # 5, 10
#
...
#     return scores, model
#
# scores, model = rf_pipe(df_train, X, y, stratified_label='Group_Age_SEX')
...
# %%
print(f"\nDone! Random Forest pipeline for {target} prediction by {feature_comb} completed successfully.")
# print Date and Time
