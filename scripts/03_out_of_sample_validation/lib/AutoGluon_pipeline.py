...
class AdMLPipeline:
    ...
    def autogluon_pipe(self):
        ...
        #         num_cpus=self.num_cores,
        #         # save_space=True,
        #         time_limit=3600,  # only for testing
        #         verbosity=3,
        #     )
        #
        # end_dask = time.time()
        ...
        # ------------------- No HTCondor parallelization version -------------------
        self.model = TabularPredictor(
            label=self.y,
            problem_type='regression',
            eval_metric='r2',
            ...
            train_data=self.df_train[self.X + [self.y]],
            presets='best_quality',  # best_quality={‘auto_stack’: True, ‘dynamic_stacking’: ‘auto’, ‘hyperparameters’: ‘zeroshot’}
            # auto_stack=True,
            # Automatically sets `num_bag_folds` and `num_stack_levels` arguments based on dataset properties.
            # num_bag_folds=5,
            # num_bag_sets=10,
    ...
    def test_perform_plot(self, df_test, age_poly=None, age_lr=None,fig_title=None, scatter_color=None):
        if 'Age_at_Scan' in self.X and 'rgo_age' in self.y:
            self.y = self.y.replace('_rgo_age', '_Test')
            self.rgo_age = True
            self.X.remove('Age_at_Scan')
        ...
        if self.rgo_age:
            self.whole_age_poly = age_poly
            self.whole_age_lr = age_lr
        ...
        # model.evaluate need train_data's target label, but it's not same in the test data
        # the model.evaluate can't be used here
        ...
        # get validation score is also not possible
        ...
        if scatter_color is not None:
            lm = sns.lmplot(x='True Values', y='Predicted Values', data=data, ci=None, aspect=1.3, height=7, scatter_kws={'color': scatter_color, 'edgecolor': 'black'}, line_kws={'color': scatter_color, 'linewidth': 2})
        else:
            lm = sns.lmplot(x='True Values', y='Predicted Values', data=data, ci=None, aspect=1.3, height=7, scatter_kws={'color': 'blue', 'edgecolor': 'black'}, line_kws={'color': 'blue', 'linewidth': 2})
        ...
        if self.save_path:
            ...
            # save as svg
            plt.savefig(self.save_path + fig_title + "_jointplot.svg", format='svg')
        plt.show()
        plt.close()

        return mae, rmse, r2, corr, corr_p, corr_spearman, corr_spearman_p, y_pred, y_true
