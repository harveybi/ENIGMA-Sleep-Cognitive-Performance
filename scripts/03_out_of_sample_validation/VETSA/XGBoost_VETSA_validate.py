...
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/Results/XGBoost/VETSA/{target}/{feature_comb}/'  # where the results will be saved

if 'Memory' in target:
    if 'rgo_age' in target:
        trained_models_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/models/XGBoost/Memory_rgo_age/{feature_comb}/'
    else:
        trained_models_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/models/XGBoost/Memory/{feature_comb}/'  # where the trained models are saved
else:
    trained_models_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/models/XGBoost/{target}/{feature_comb}/'  # where the trained models are saved
...
# %%
# # rename Memory_Test_Digit to Memory_Digit_Test, and Memory_Test_Letter to Memory_Letter_Test
# df_VETSA.rename(columns={
#     'Memory_Test_Digit': 'Memory_Digit_Test',
#     'Memory_Test_Letter': 'Memory_Letter_Test'
# }, inplace=True)
#
# # %%
# # save back
# df_VETSA.to_csv(data_save_path + 'VETSA_dataset_renamed_target_cleaned.csv', index=False)

# %%
# Add new columns PSG_Sleep_Dur and PSG_Sleep_Eff with NaN values
...
test_mae_VETSA, test_rmse_VETSA, test_r2_VETSA, test_corr_VETSA, test_corr_p_VETSA, test_corr_spearman_VETSA, test_corr_spearman_p_VETSA, VETSA_pred, VETSA_true = ml_pipe_instance.test_perform_plot(df_test_VETSA, X_train_preprocessor=whole_X_train_preprocessor, age_poly=whole_age_poly, age_lr=whole_age_lr, fig_title=f'San Diego', scatter_color='#E4BDE5')
...
results_values_df = df_test_VETSA['Age_at_Scan']
results_values_df['VETSA_pred'] = VETSA_pred
results_values_df['VETSA_true'] = VETSA_true
...
print(f"\nDone! XGBoost pipeline for {target} prediction by {feature_comb} in VETSA completed successfully.")
