...
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/Results/AutoGluon/Liege/{target}/{feature_comb}/'  # where the results will be saved
...
numeric_features = [feature for feature in X if df_test_Liege[feature].dtype.name in ['int64', 'float64']]
...
# %%
test_mae_Liege, test_rmse_Liege, test_r2_Liege, test_corr_Liege, test_corr_p_Liege, test_corr_spearman_Liege, test_corr_spearman_p_Liege, Liege_pred, Liege_true = ml_pipe_instance.test_perform_plot(df_test_Liege, age_poly=whole_age_poly, age_lr=whole_age_lr, fig_title=f'Liège', scatter_color='#56BBB8')
...
results_values_df = df_test_Liege[['Age_at_Scan']].copy()  # Explicitly create a new DataFrame
results_values_df['Liege_pred'] = Liege_pred  # Add predictions
results_values_df['Liege_true'] = Liege_true  # Add true values
...
print(f"\nDone! AutoGluon pipeline for {target} prediction by {feature_comb} in Liege completed successfully.")
