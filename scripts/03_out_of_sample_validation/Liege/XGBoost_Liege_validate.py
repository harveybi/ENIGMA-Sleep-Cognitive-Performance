...
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/Results/XGBoost/Liege/{target}/{feature_comb}/'  # where the results will be saved
...
print(f"Range of Stroop_Test: {df_Liege['Stroop_Test'].min()} - {df_Liege['Stroop_Test'].max()}")
print(f"Range of Memory_Test: {df_Liege['Memory_Test'].min()} - {df_Liege['Memory_Test'].max()}")
...
test_mae_Liege, test_rmse_Liege, test_r2_Liege, test_corr_Liege, test_corr_p_Liege, test_corr_spearman_Liege, test_corr_spearman_p_Liege, Liege_pred, Liege_true = ml_pipe_instance.test_perform_plot(df_test_Liege, X_train_preprocessor=whole_X_train_preprocessor, age_poly=whole_age_poly, age_lr=whole_age_lr, fig_title=f'Liège', scatter_color='#56BBB8')
...
results_metrics_df = pd.DataFrame({
    ...
    'Performance Test Set Pearson r p-value Liege': [test_corr_p_Liege],
    'Performance Test Set Spearman r Liege': [test_corr_spearman_Liege],
    'Performance Test Set Spearman r p-value Liege': [test_corr_spearman_p_Liege]
})

# make a dataframe results_values_df, columns are 'Age_at_Scan', 'Liege_pred', 'Liege_true'
results_values_df = df_test_Liege[['Age_at_Scan']]
results_values_df['Liege_pred'] = Liege_pred
results_values_df['Liege_true'] = Liege_true
...
print(f"\nDone! XGBoost pipeline for {target} prediction by {feature_comb} in Liege completed successfully.")
