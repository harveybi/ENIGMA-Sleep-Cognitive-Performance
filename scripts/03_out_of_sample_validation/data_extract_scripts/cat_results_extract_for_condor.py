...
# %%
control_save_path = '/data/project/sleep_ENIGMA_insomnia/Data/KUMS_Iran_20.07.2021/KUMS/controlbids/CAT12.8.1/'
patient_save_path = '/data/project/sleep_ENIGMA_insomnia/Data/KUMS_Iran_20.07.2021/KUMS/patientbids/CAT12.8.1/'

# load csv ROI file, ROI_Schaefer2018_400Parcels_17Networks_order_Vgm.csv
control_roi_df = pd.read_csv(control_save_path + 'Results/' + 'ROI_Schaefer2018_400Parcels_17Networks_order_Vgm.csv')
patient_roi_df = pd.read_csv(patient_save_path + 'Results/' + 'ROI_Schaefer2018_400Parcels_17Networks_order_Vgm.csv')
...
# %%
control_covariates_df = get_covariates(control_save_path)
patient_covariates_df = get_covariates(patient_save_path)

# %%
# get the roi_df without the first column 'names'
control_roi_df_need = control_roi_df.iloc[:, 1:]
patient_roi_df_need = patient_roi_df.iloc[:, 1:]
# append roi_df to covariates_df, but for roi_df we not need the first column 'name'
control_covariates_df_all = pd.concat([control_covariates_df, control_roi_df_need], axis=1)
patient_covariates_df_all = pd.concat([patient_covariates_df, patient_roi_df_need], axis=1)

# %%
# save the dataframe as csv file
control_covariates_df_all.to_csv(control_save_path + 'Results/' + 'KUMS_control_cat12.8.1_1surf_rois_Schaefer2018_400Parcels_17Networks_order.csv', index=False)
patient_covariates_df_all.to_csv(patient_save_path + 'Results/' + 'KUMS_patient_cat12.8.1_1surf_rois_Schaefer2018_400Parcels_17Networks_order.csv', index=False)