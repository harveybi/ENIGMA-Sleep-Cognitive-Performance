...
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/Results/XGBoost/KI/{target}/{feature_comb}/'  # where the results will be saved
...
test_mae_KI, test_rmse_KI, test_r2_KI, test_corr_KI, test_corr_p_KI, test_corr_spearman_KI, test_corr_spearman_p_KI, KI_pred, KI_true = ml_pipe_instance.test_perform_plot(df_test_KI, X_train_preprocessor=whole_X_train_preprocessor, age_poly=whole_age_poly, age_lr=whole_age_lr, fig_title=f'Stockholm', scatter_color='#7EA8D3')
...
results_values_df = df_test_KI[['Age_at_Scan']]
results_values_df['KI_pred'] = KI_pred
results_values_df['KI_true'] = KI_true
...
print(f"\nDone! XGBoost pipeline for {target} prediction by {feature_comb} in KI completed successfully.")
