...
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/Results/XGBoost/EMC/{target}/{feature_comb}/'  # where the results will be saved
...
test_mae_EMC, test_rmse_EMC, test_r2_EMC, test_corr_EMC, test_corr_p_EMC, test_corr_spearman_EMC, test_corr_spearman_p_EMC, EMC_pred, EMC_true = ml_pipe_instance.test_perform_plot(df_test_EMC, X_train_preprocessor=whole_X_train_preprocessor, age_poly=whole_age_poly, age_lr=whole_age_lr, fig_title=f'Rotterdam', scatter_color='#F3A3A7')
...
results_values_df = df_test_EMC['Age_at_Scan']
results_values_df['EMC_pred'] = EMC_pred
results_values_df['EMC_true'] = EMC_true
...
print(f"\nDone! XGBoost pipeline for {target} prediction by {feature_comb} in EMC completed successfully.")
