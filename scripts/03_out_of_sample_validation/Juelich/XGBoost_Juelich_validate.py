...
print(f"\nStarting XGBoost out-of-sample validation pipeline for session {sess}, {target} prediction with feature combination {feature_comb}.\n")

# %%
data_save_path = '/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Data/'  # where the data is saved
results_path = f'/data/project/sleep_ENIGMA_Cognition/Codes/ENIGMA_Sleep_Cognitive/Code/Out-of-sample_validation/Results/XGBoost/Juelich/{sess}/{target}/{feature_comb}/'  # where the results will be saved
...
#
# # for column 'Executive_Functioning', 'Memory_Test1', 'Memory_Test2' in df_Juelich, rename 'Executive_Functioning' to 'Executive_Functioning_Test', rename 'Memory_Test1' to 'Memory_Letter_Test', rename 'Memory_Test2' to 'Memory_Spatial_Test'
# df_Juelich_1 = df_Juelich_1.rename(columns={'Letter_Sensitivity': 'Letter_Sensitivity_Test', 'Spatial_Sensitivity': 'Spatial_Sensitivity_Test'})
# df_Juelich_2 = df_Juelich_2.rename(columns={'Letter_Sensitivity': 'Letter_Sensitivity_Test', 'Spatial_Sensitivity': 'Spatial_Sensitivity_Test'})
# df_Juelich_SD = df_Juelich_SD.rename(columns={'Letter_Sensitivity': 'Letter_Sensitivity_Test', 'Spatial_Sensitivity': 'Spatial_Sensitivity_Test'})
#
# # save back
# df_Juelich_1.to_csv(data_save_path + 'Juelich_SomnoSafe_1_target_cleaned.csv')
# df_Juelich_2.to_csv(data_save_path + 'Juelich_SomnoSafe_2_target_cleaned.csv')
# df_Juelich_SD.to_csv(data_save_path + 'Juelich_SomnoSafe_SD_target_cleaned.csv')
...
# %%
columns_to_int = ['SEX']
df_Juelich_ml[columns_to_int] = df_Juelich_ml[columns_to_int].astype('int64')
...
# %%
cat_dtype_saved_path = trained_models_path + 'cat_dtypes.pkl'
with open(cat_dtype_saved_path, 'rb') as f:
    cat_dtypes = pickle.load(f)

# apply cat_dtypes to convert data types of columns
columns_to_object = ['SEX', 'APOE4']
for col, dtype in cat_dtypes.items():
    df_Juelich_ml[col] = pd.Categorical(df_Juelich_ml[col], dtype.categories)
...
results_values_df = df_test_Juelich['Age_at_Scan']
results_values_df['Juelich_pred'] = Juelich_pred
results_values_df['Juelich_true'] = Juelich_true
...
print(f"\nDone! XGBoost pipeline for {target} prediction by {feature_comb} in Juelich completed successfully.")
